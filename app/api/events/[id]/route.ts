...
export async function GET(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (error) {
    if ((error as any).code === 'PGRST116') return jsonError('not_found', 'Event not found', 404)
    return jsonError('internal', error.message, 500)
  }

  // Optional: include viewer RSVP if authed
  let viewerRsvp: { status: string; createdAt: string } | null = null
  try {
    const jar = await cookies()
    const token = jar.get('mainey_access_token')?.value
    if (token) {
      ...
      if (userId) {
        const r = await supabase
          ...
          .eq('user_id', userId)
          .maybeSingle()
        if (r.data) viewerRsvp = { status: r.data.status, createdAt: r.data.created_at }
      }
    }
  } catch {
    viewerRsvp = null
...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const existing = await anon.from('events').select('owner_id').eq('id', id).maybeSingle()
  if (existing.error) return jsonError('internal', existing.error.message, 500)
  if (!existing.data) return jsonError('not_found', 'Event not found', 404)
  if (existing.data.owner_id !== userId) return jsonError('forbidden', 'Only the event owner can edit', 403)
